"""Media source implementation for the Ambient Sound Synthesizer integration."""
...
from .stream import SoundStreamManager, SoundStreamProfile
...
async def async_get_media_source(hass: HomeAssistant) -> "AmbientSoundMediaSource":
    ...
    source: AmbientSoundMediaSource | None = domain_data.get("media_source")
    if source is None:
        source = AmbientSoundMediaSource(hass)
        domain_data["media_source"] = source
    return source


class AmbientSoundMediaSource(MediaSource):
    """Expose configured ambient sound profiles as a Home Assistant media source."""

    name = "Ambient Sound Synthesizer"

    def __init__(self, hass: HomeAssistant) -> None:
        super().__init__(DOMAIN)
        self.hass = hass

    def _single_entry(self) -> tuple[str, SoundStreamManager] | None:
        domain = self.hass.data.get(DOMAIN, {})
        entries = domain.get("entries", {})
        for stored in entries.values():
            manager: SoundStreamManager | None = stored.get("manager")
            if manager is None:
                continue
    ...
    async def async_browse_media(self, item: MediaSourceItem) -> Any:
        ...
        if entry is None:
            raise Unresolvable("Ambient Sound Synthesizer is not configured")
    ...
    async def async_resolve_media(self, item: MediaSourceItem) -> PlayMedia:
        ...
        entry = self._single_entry()
        if entry is None:
            raise MediaSourceError("Ambient Sound Synthesizer is not configured")

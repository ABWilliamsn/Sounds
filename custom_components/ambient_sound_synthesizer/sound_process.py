"""Subprocess entry-point that streams ambient sounds as WAV PCM data."""
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Ambient sound streaming process")
    parser.add_argument("--mode", required=True, choices=PROFILE_TYPES + ["noise", "ambient"])
...
def _coerce_seed(seed: Any | None) -> Any | None:
    """Convert seed to appropriate type."""
...
def run(argv: list[str]) -> int:
    ...
    _LOGGER.info(
        "Starting ambient_sound_process mode=%s subtype=%s volume=%s params=%s",
        args.mode,
        args.subtype,
        args.volume,
        parameters,
    ...
    _LOGGER.info("Exiting ambient_sound_process mode=%s subtype=%s", args.mode, args.subtype)
    return 0


def main() -> None:
    """Entry point for the streaming process."""

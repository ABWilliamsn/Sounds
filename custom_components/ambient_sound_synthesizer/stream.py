"""Streaming helpers for the Ambient Sound Synthesizer integration."""
...
@dataclass
class SoundStreamProfile:
    """Representation of a playable sound profile."""
...
class SoundStreamView(HomeAssistantView):
    """Serve streaming audio responses for configured sound profiles."""

    url = f"{STREAM_URL_PATH}/{{entry_id}}/{{slug}}"
    name = "api:ambient_sound_synthesizer:stream"
    requires_auth = False
...
class SoundStreamManager:
    """Manage runtime state for streaming sound profiles."""

    def __init__(self, hass: HomeAssistant, entry_id: str) -> None:
        self.hass = hass
        self.entry_id = entry_id
        self._profiles: dict[str, SoundStreamProfile] = {}
    ...
    def update_profiles(self, profiles: list[dict[str, Any]]) -> None:
        ...
        new_profiles: dict[str, SoundStreamProfile] = {}
        ...
        for index, profile in enumerate(profiles):
            ...
            new_profiles[slug_candidate] = SoundStreamProfile(
                slug=slug_candidate,
                name=name,
    ...
    async def async_stream_profile(
        self, request: web.Request, profile: SoundStreamProfile
    ) -> web.StreamResponse:
    ...
    async def _create_process_handle(self, profile: SoundStreamProfile) -> _BaseStreamHandle:
        ...
        stderr_task = self.hass.async_create_task(
            ...
            name=f"ambient_sound_synthesizer_stderr_{profile.slug}",
        )
        handle = _ProcessStreamHandle(self, process, stderr_task)
        self._handles.add(handle)
        return handle

    async def _launch_process(self, profile: SoundStreamProfile) -> asyncio.subprocess.Process:
        ...
        args = [
            sys.executable,
            "-m",
            "custom_components.ambient_sound_synthesizer.sound_process",
            "--mode",
            profile_type,
            "--subtype",
            profile_subtype,
